...
async function sendContactEmail({ firstName, lastName, email, subject, message }) {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.RESEND_FROM_EMAIL || 'Nink <hello@nink.com>';
  const notifyTo = process.env.CONTACT_NOTIFY_EMAIL || process.env.WAITLIST_NOTIFY_EMAIL || 'peter@nink.com';
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111;max-width:560px">
      <p style="font-weight:700;margin:0 0 12px">NINK website contact form</p>
      <p style="margin:0"><strong>From:</strong> ${safeFirst} ${safeLast}</p>
      <p style="margin:8px 0 0"><strong>Email:</strong> <a href="mailto:${safeEmail}">${safeEmail}</a></p>
  ...
  return sendResendEmail({
    ...
    to: [notifyTo],
    replyTo: email,
    subject: `NINK contact: ${subject}`,
    html
  });
}

module.exports = { sendVerificationEmail, sendSignupNotificationEmail, sendContactEmail };
