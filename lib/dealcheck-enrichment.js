...
async function runFlyerDedupeAfterIngest(options = {}) {
  const base = (process.env.DEALCHECK_URL || 'https://dealcheck.nink.com').replace(/\/$/, '');
  const token = process.env.CRON_SECRET;
  ...
  const params = new URLSearchParams({ confirm: 'true', token });
  if (options.retailer) params.set('retailer', options.retailer);

  const response = await fetch(`${base}/api/cleanup-flyer-duplicates?${params.toString()}`);
  const data = await response.json().catch(() => ({}));
  if (!response.ok || !data.ok) {
    throw new Error(data.error || `DealCheck flyer dedupe failed (${response.status})`);
  }
...
module.exports = { runFlyerEnrichmentAfterIngest, runFlyerLocalImagesAfterIngest, runFlyerDedupeAfterIngest };
