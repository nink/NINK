...
const DEFAULT_POSTAL = process.env.FLIPP_POSTAL_CODE || 'L4N6B7';
...
async function fetchFlyers(postal = DEFAULT_POSTAL) {
  const data = await fetchJson(`${FLIPP_BASE}/data?locale=en-ca&postal_code=${encodeURIComponent(postal)}`);
  return data.flyers || [];
}

function pickWeeklyFlyer(flyers, merchantId) {
  const candidates = flyers.filter((f) => {
    if (f.merchant_id !== merchantId) return false;
    const cats = f.categories || [];
    return cats.includes('Groceries') || cats.some((c) => /grocer/i.test(c));
  });

  const weekly = candidates.filter((f) => /weekly|eflyer|flyer|ad/i.test(String(f.name || '')));
...
async function fetchFlyerItems(postal, merchantId, flyerId, groceryOnly = false) {
  ...
  for (const term of SEARCH_TERMS) {
    const url =
      `${FLIPP_BASE}/items/search?locale=en-ca&postal_code=${encodeURIComponent(postal)}` +
      `&merchant_id=${merchantId}&q=${encodeURIComponent(term)}`;
    const data = await fetchJson(url);

    for (const item of data.items || []) {
      if (item.flyer_id !== flyerId) continue;
...
async function ingestStore(storeKey, postal = DEFAULT_POSTAL) {
  ...
  const flyers = await fetchFlyers(postal);
  const flyer = pickWeeklyFlyer(flyers, config.merchantId);
  if (!flyer) {
    return { ok: false, store: storeKey, retailer, error: 'No active weekly grocery flyer found on Flipp' };
  }

  const items = await fetchFlyerItems(postal, config.merchantId, flyer.id, !!config.groceryOnly);
  const deals = items
